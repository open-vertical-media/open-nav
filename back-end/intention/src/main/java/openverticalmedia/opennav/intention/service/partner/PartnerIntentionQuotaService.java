...
public class PartnerIntentionQuotaService {
    ...
    public Pager<PartnerIntentionQuotaDto> query(ManagerModel manager, String type, int page, int size) {
        Pager<PartnerIntentionQuotaDto> result = new Pager<>();
        Page<IntentionQuotaEntity> entityPage;
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        if (StrUtil.isNotBlank(type)) {
            entityPage = repository.queryByPartnerIdAndType(manager.getId(), type, pageRequest);
        } else {
            entityPage = repository.queryByPartnerId(manager.getId(), pageRequest);
        }
